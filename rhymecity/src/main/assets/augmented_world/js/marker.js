...
function Marker(poiData) {
	 ...
	 * ARchitect objects. This sample will animate the opacity of both
	 * background drawables so that one will fade out while the other one fades
	 * in. The scaling is animated too. The marker size changes over time so the
	 * labels need to be animated too in order to keep them relative to the
	 * background drawable. AR.AnimationGroups are used to synchronize all
	...
	console.log(poiData.image);
	World.markerDrawable_idle = new AR.ImageResource(poiData.image);
	World.markerDrawable_selected = new AR.ImageResource(poiData.image);

	// new AR.ImageResource(poiData.image)
	 ...
	 * AR.Drawable. The property is a function which will be called each time
	 * the user taps on the drawable. The function called on each tap is
	 * returned from the following helper function defined in marker.js. The
	 * function returns a function which checks the selected state with the help
	 * of the variable isSelected and executes the appropriate function. The
	 * clicked marker is passed as an argument.
	 */
	...
	/*
	 * Create an AR.ImageDrawable using the AR.ImageResource for the direction
	 * indicator which was created in the World. Set options regarding the
	 * offset and anchor of the image so that it will be displayed correctly on
...
Marker.prototype.getOnClickTrigger = function(marker) {
	...
	/*
	 * The setSelected and setDeselected functions are prototype Marker
	 * functions. Both functions perform the same steps but inverted.
	 */
...
/*
 * Property Animations allow constant changes to a numeric value/property of an
 * object, dependent on start-value, end-value and the duration of the
 * animation. Animations can be seen as functions defining the progress of the
...
Marker.prototype.setSelected = function(marker) {
	...
	if (marker.animationGroup_selected === null) {
		...
		// create AR.PropertyAnimation that animates the opacity to 0.0 in order
		// to hide the idle-state-drawable
		...
		// create AR.PropertyAnimation that animates the opacity to 1.0 in order
		// to show the selected-state-drawable
		...
		// create AR.PropertyAnimation that animates the scaling of the
		// idle-state-drawable to 1.2
		...
		// create AR.PropertyAnimation that animates the scaling of the
		// selected-state-drawable to 1.2
		...
		// create AR.PropertyAnimation that animates the scaling of the title
		// label to 1.2
		...
		/*
		 * There are two types of AR.AnimationGroups. Parallel animations are
		 * running at the same time, sequentials are played one after another.
	...
	// removes function that is set on the onClick trigger of the idle-state
	// marker
	marker.markerDrawable_idle.onClick = null;
	// sets the click trigger function for the selected state marker
	marker.markerDrawable_selected.onClick = Marker.prototype
...
Marker.prototype.setDeselected = function(marker) {
	...
	if (marker.animationGroup_idle === null) {
		...
		// create AR.PropertyAnimation that animates the opacity to 1.0 in order
		// to show the idle-state-drawable
		...
		// create AR.PropertyAnimation that animates the opacity to 0.0 in order
		// to hide the selected-state-drawable
		...
		// create AR.PropertyAnimation that animates the scaling of the
		// selected-state-drawable to 1.0
	...
	marker.markerDrawable_idle.onClick = Marker.prototype
			.getOnClickTrigger(marker);
	// removes function that is set on the onClick trigger of the selected-state
	// marker
...
Marker.prototype.isAnyAnimationRunning = function(marker) {
	...
	if (marker.animationGroup_idle === null || marker.animationGroup_selected === null) {
		...
		if ((marker.animationGroup_idle.isRunning() === true) || (marker.animationGroup_selected
				.isRunning() === true)) {
			return true;
		} else {
			return false;
