...
var World = {
	// you may request new data from server periodically, however: in this
	// sample data is only requested once
...
//		World.markerDrawable_idle = new AR.ImageResource("http://talkingbiznews.com/wp-content/uploads/2013/04/Nike-logo.jpg");
//		World.markerDrawable_selected = new AR.ImageResource("http://talkingbiznews.com/wp-content/uploads/2013/04/Nike-logo.jpg");
		World.markerDrawable_directionIndicator = new AR.ImageResource("assets/indi.png");

		// loop through POI-information and create an AR.GeoObject (=Marker) per
		// POI
		for (var currentPlaceNr = 0; currentPlaceNr < poiData.length; currentPlaceNr++) {
			var singlePoi = {
				...
				"image" : poiData[currentPlaceNr].image_path
//				"image" : poiData[currentPlaceNr].image_url
			};
	...
	// sets/updates distances of all makers so they are available way faster
	// than calling (time-consuming) distanceToUser() method all the time
	...
	// location updates, fired every time you call architectView.setLocation()
...
//
//		var distanceToUserValue = (marker.distanceToUser > 999) ? ((marker.distanceToUser / 1000)
//				.toFixed(2) + " km")
...
//		});
	...
	onScreenClick : function onScreenClickFn() {
		...
		var architectSdkUrl = "architectsdk://clearmarkerselected?";
	...
	getMaxDistance : function getMaxDistanceFn() {
		...
		// sort palces by distance so the first entry is the one with the
		// maximum distance
		...
		// return maximum distance times some factor >1.0 so ther is some room
		// left and small movements of user don't cause places far away to
		// disappear
	...
	*/
	...
	onPoiDetailMoreButtonClicked: function onPoiDetailMoreButtonClickedFn() {
		...
		var distanceToUserValue = (currentMarker.distanceToUser > 999) ? ((currentMarker.distanceToUser / 1000).toFixed(2) + " km") : (Math.round(currentMarker.distanceToUser) + " m");

		var architectSdkUrl = "architectsdk://markerselected?"
				+ "id=" + encodeURIComponent(currentMarker.poiData.id) 
				...
				+ "&distance=" + encodeURIComponent(distanceToUserValue);
		/*
